...
public class Swimmer extends Thread {
	...
	public void run() {
		try {
			...
			// * Signal that swimmer is ready
			synchronized (this) {
				notify(); // * Notify any thread waiting for this swimmer to be ready
			}

			// Move to starting blocks
			goToStartingBlocks();

			// * Synchronize the start of the race for the team
			synchronized (stadium) {
				while (stadium.getTeam(team)) {
					stadium.wait(); // * Wait if another swimmer from the same team is swimming
				}
				stadium.setTeam(team, true); // * Mark the team as active, allowing the next swimmer to start
			}

			// * Use latch to ensure all teams are ready before starting
			stadium.latch.countDown(); // * Decrease latch count by 1
			stadium.latch.await(); // * Wait until all teams are ready to start

			dive();

			swimRace();

			// Once done swimming, allow the next swimmer to start
			synchronized (stadium) {
				stadium.setTeam(team, false); // * Mark team as not swimming
				stadium.notifyAll();
			}

			// * If this is the last swimmer (freestyle), finish the race
